    ...
    try:
        ...
        if 'keyword' in queries.columns or 'search' in queries.columns:
            ...
            with st.spinner('🔥 Analyzing top health keywords by subcategory...'):
                ...
                @st.cache_data(ttl=1800, show_spinner=False)
                def compute_subcategory_health_performance_monthly(_df, _sc, month_names_dict, cache_key):
                    """🔄 FIXED: Proper monthly CTR/CR calculations for health subcategories"""
                    if _df.empty or _sc.empty:
                        return pd.DataFrame(), []
                    
                    # Get top subcategories by total counts
                    top_subcategories_list = _sc.nlargest(50, 'Counts')['sub_category'].tolist()
                    
                    # Filter original data for top subcategories
                    ...
                    for subcategory in top_subcategories_list:
                        ...
                        # ✅ SKIP SUBCATEGORIES WITH NO DATA
                        ...
                        # ✅ SKIP SUBCATEGORIES WITH ZERO VOLUME
                        if total_counts == 0:
                            continue
                ...
                top_subcategories_monthly, unique_months_sub = compute_subcategory_health_performance_monthly(queries, sc, month_names, subcategory_filter_key)
                
                if top_subcategories_monthly.empty:
                    st.warning("No valid subcategory data after processing.")
                else:
                    # ✅ CLEAN DATA: Remove any remaining empty rows and limit to selected number
                    top_subcategories_monthly = top_subcategories_monthly.dropna(subset=['Health Subcategory'])
                    top_subcategories_monthly = top_subcategories_monthly[top_subcategories_monthly['Total Volume'] > 0]
                    top_subcategories_monthly = top_subcategories_monthly.head(num_subcategories)
                    
                    # 🔄 BETTER ARRANGEMENT: Reorder columns for logical flow
                    base_columns = ['Health Subcategory', 'Total Volume', 'Share %', 'Overall CTR', 'Overall CR', 'Total Clicks', 'Total Conversions']
                    ...
                    for month in sorted(unique_months_sub):
                        month_display = month_names.get(month, month)
                        volume_columns.append(f'{month_display} Vol')
                        ctr_columns.append(f'{month_display} CTR')
                        cr_columns.append(f'{month_display} CR')
                    ...
                    if ('styled_subcategories_health' not in st.session_state or 
                        st.session_state.get('subcategories_health_cache_key') != subcategories_hash):
                        
                        st.session_state.subcategories_health_cache_key = subcategories_hash
                        ...
                        def highlight_subcategory_health_performance_with_comparison(df):
                            """Enhanced highlighting for better health subcategory comparison"""
                            styles = pd.DataFrame('', index=df.index, columns=df.columns)
                            
                            if len(unique_months_sub) < 2:
                                return styles
                            
                            sorted_months = sorted(unique_months_sub)
                    ...
                    # 🔄 ENHANCED: Column grouping explanation for subcategories
                    ...
                    with col_download[1]:
                        st.download_button(
                            label="📥 Download Nutraceuticals & Nutrition Subcategories CSV",
                            data=csv_subcategories,
                            file_name=f"top_{num_subcategories}_health_subcategories_monthly_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.csv",
                            mime="text/csv",
                            help="Download the health subcategories table with monthly comparison data",
                            use_container_width=True,
        ...
        if analysis_type == "📊 Top Health Performers Overview":
                    ...
                    if 'keyword' in queries.columns or 'search' in queries.columns:
                        ...
                        if len(subcat_keywords) > 0:
                            ...
                            fig_keywords = px.bar(
                                ...
                                labels={'Counts': 'Health Search Volume', keyword_col: 'Health Keywords'},
                                color='keyword_ctr',
                                color_continuous_scale=['#E8F5E8', '#81C784', '#2E7D32'],
                                text='Counts'
                            )
            ...
            selected_subcategories = st.multiselect(
                f"Select health subcategories to compare (max {max_selections}):",
                options=sc['sub_category'].tolist(),
            ...
            if selected_subcategories:
                with st.spinner('⚖️ Comparing selected health subcategories...'):
                    ...
                    comparison_table['Health Search Volume'] = comparison_table['Health Search Volume'].apply(lambda x: format_number(int(x)))
                    comparison_table['Health Clicks'] = comparison_table['Health Clicks'].apply(lambda x: format_number(int(x)))
                    comparison_table['Nutraceuticals & Nutrition Conversions'] = comparison_table['Nutraceuticals & Nutrition Conversions'].apply(lambda x: format_number(int(x)))
            ...
            with st.spinner('📊 Generating market share analysis...'):
                ...
                with col_pie:
                    ...
                    fig_pie = px.pie(
                        ...
                        values='Counts',
                        names='sub_category',
                        title=f'<b style="color:#2E7D32;">🌿 Top {display_count} Health Subcategories Market Share</b>',
                ...
                with col_treemap:
                    ...
                    fig_treemap = px.treemap(
                        ...
                        path=['sub_category'],
                        values='Counts',
                        title=f'<b style="color:#2E7D32;">🌿 Health Subcategory Volume Distribution (Top {treemap_count})</b>',
                        color='ctr',
                        color_continuous_scale=['#E8F5E8', '#81C784', '#2E7D32'],
...
with tab_generic:
    ...
    try:
        ...
        # Create a simple performance table for generic type data
        display_gt = gt_agg.head(num_generic_terms).copy()
        
        # Calculate share percentage
        total_volume = gt_agg['count'].sum()
        display_gt['Share (%)'] = (display_gt['count'] / total_volume * 100).round(2)
        
        # Rename columns for better display
        display_gt = display_gt.rename(columns={
            'search': 'Generic Health Term',
            'count': 'Total Volume',
            'Clicks': 'Total Clicks',
            ...
            'conversion_share': 'Conv Share (%)'
        })
        
        # Reorder columns for better presentation
        column_order = [
            'Generic Health Term', 'Total Volume', 'Share (%)', 
            'CTR (%)', 'Conversion Rate (%)', 'CVR (%)',
            'Total Clicks', 'Total Conversions', 
            'Click Share (%)', 'Conv Share (%)'
        ]
        display_gt = display_gt[column_order]
        
        # Format numeric columns
        display_gt_formatted = display_gt.copy()
        display_gt_formatted['Total Volume'] = display_gt_formatted['Total Volume'].apply(lambda x: format_number(int(x)))
        display_gt_formatted['Total Clicks'] = display_gt_formatted['Total Clicks'].apply(lambda x: format_number(int(x)))
        display_gt_formatted['Total Conversions'] = display_gt_formatted['Total Conversions'].apply(lambda x: format_number(int(x)))
        
        # Enhanced styling with performance-based highlighting
        def highlight_generic_performance(df):
            """Highlight performance metrics based on thresholds"""
            styles = pd.DataFrame('', index=df.index, columns=df.columns)
            
            # CTR highlighting
            if 'CTR (%)' in df.columns:
                for idx in df.index:
                    ctr_val = df.loc[idx, 'CTR (%)']
                    if pd.notnull(ctr_val):
                        if ctr_val >= 8:  # High CTR
                            styles.loc[idx, 'CTR (%)'] = 'background-color: rgba(76, 175, 80, 0.3); color: #1B5E20; font-weight: bold;'
                        elif ctr_val >= 5:  # Medium CTR
                            styles.loc[idx, 'CTR (%)'] = 'background-color: rgba(76, 175, 80, 0.15);'
                        elif ctr_val < 2:  # Low CTR
                            styles.loc[idx, 'CTR (%)'] = 'background-color: rgba(244, 67, 54, 0.15);'
            
            # Conversion Rate highlighting
            if 'Conversion Rate (%)' in df.columns:
                for idx in df.index:
                    cr_val = df.loc[idx, 'Conversion Rate (%)']
                    if pd.notnull(cr_val):
                        if cr_val >= 3:  # High CR
                            styles.loc[idx, 'Conversion Rate (%)'] = 'background-color: rgba(76, 175, 80, 0.3); color: #1B5E20; font-weight: bold;'
                        elif cr_val >= 1.5:  # Medium CR
                            styles.loc[idx, 'Conversion Rate (%)'] = 'background-color: rgba(76, 175, 80, 0.15);'
                        elif cr_val < 0.5:  # Low CR
                            styles.loc[idx, 'Conversion Rate (%)'] = 'background-color: rgba(244, 67, 54, 0.15);'
        ...
        # Apply styling
        styled_gt = display_gt_formatted.style.apply(highlight_generic_performance, axis=None)
        
        styled_gt = styled_gt.format({
            'Share (%)': '{:.2f}%',
            'CTR (%)': '{:.2f}%',
            'Conversion Rate (%)': '{:.2f}%',
            'CVR (%)': '{:.2f}%',
        ...
        actual_rows = len(display_gt_formatted)
        table_height = min(max(actual_rows * 45 + 60, 250), 650)
        ...
        st.markdown("""
        <div style="background: rgba(46, 125, 50, 0.1); padding: 12px; border-radius: 8px; margin: 15px 0;">
            <h4 style="margin: 0 0 8px 0; color: #1B5E20;">🌿 Performance Indicators:</h4>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px;">
                ...
                <div>⚠️ <strong style="background-color: rgba(244, 67, 54, 0.15); padding: 2px 6px; border-radius: 4px;">Light Red</strong> = Needs Attention</div>
        ...
        st.markdown("""
        <div style="background: rgba(46, 125, 50, 0.1); padding: 10px; border-radius: 8px; margin: 10px 0;">
            <h4 style="margin: 0 0 8px 0; color: #1B5E20;">📊 Column Definitions:</h4>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 15px; font-size: 0.9em;">
                <div><strong>🌱 CTR (%):</strong> Click-through rate (Clicks ÷ Volume)</div>
                <div><strong>💚 Conversion Rate (%):</strong> Conversion rate based on searches</div>
                <div><strong>🎯 CVR (%):</strong> Conversion rate based on clicks</div>
                <div><strong>📈 Share (%):</strong> Percentage of total search volume</div>
                <div><strong>🖱️ Click Share (%):</strong> Percentage of total clicks</div>
                <div><strong>✅ Conv Share (%):</strong> Percentage of total conversions</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        # Download section
        st.markdown("<br>", unsafe_allow_html=True)
        csv_data = gt_agg.head(num_generic_terms).to_csv(index=False)
