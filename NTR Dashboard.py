...
with tab_search:
    ...
    # Calculate the missing variables BEFORE they're used
    try:
        # Calculate long tail percentage (queries with 3+ words)
        if 'query_length' in queries.columns:
            long_tail_queries = queries[queries['query_length'] >= 3]
            long_tail_pct = (len(long_tail_queries) / len(queries)) * 100 if len(queries) > 0 else 0
        else:
            # Fallback: calculate from keywords column
            queries['query_length'] = queries['keywords'].str.split().str.len()
            long_tail_queries = queries[queries['query_length'] >= 3]
            long_tail_pct = (len(long_tail_queries) / len(queries)) * 100 if len(queries) > 0 else 0
    ...
    with col_m1:
        unique_queries = queries['normalized_query'].nunique() if 'normalized_query' in queries.columns else queries['keywords'].nunique()
        st.markdown(f"""
        <div class='mini-metric'>
            <span class='icon'>🌿</span>
    ...
    with col_m3:
        ...
        if 'Click Through Rate' in queries.columns:
            avg_ctr = queries['Click Through Rate'].mean()
            high_ctr_queries = queries[queries['Click Through Rate'] > avg_ctr]
            high_ctr_pct = (len(high_ctr_queries) / len(queries)) * 100 if len(queries) > 0 else 0
        else:
            high_ctr_pct = 0
        ...
        <div class='mini-metric'>
            <span class='icon'>📈</span>
            <div class='value'>{high_ctr_pct:.1f}%</div>
            <div class='label'>High CTR Queries</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_m4:
        ...
        <div class='mini-metric'>
            <span class='icon'>🎯</span>
            <div class='value'>{top_keyword_pct:.1f}%</div>
            <div class='label'>Top Performing</div>
        </div>
        """, unsafe_allow_html=True)
    
    # Health Search Intelligence Summary
    st.markdown("""
        <div class='insight-box'>
    ...
    with col_left:
        ...
        # Safe import of fuzzywuzzy with fallback
        ...
        # Custom similarity function as fallback
        ...
        # Calculate keyword performance with progress indicator
    ...
    with col_right:
        # Cached Query Length Distribution
