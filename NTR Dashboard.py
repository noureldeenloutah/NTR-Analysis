...
with tab_overview:
    ...
    # ✅ FIX 1: ADD HELPER FUNCTION FOR SAFE COLUMN SUMMING
    def safe_sum_column(df, col_name):
        """Safely sum a column that might be numeric or formatted string"""
        if col_name not in df.columns:
            return 0
        
        col_data = df[col_name]
        
        # If already numeric, just sum
        if pd.api.types.is_numeric_dtype(col_data):
            return int(col_data.sum())
        
        # If string (formatted), convert back to numeric
        try:
            numeric_data = col_data.astype(str).str.replace(',', '').str.replace('K', '000').str.replace('M', '000000')
            return int(pd.to_numeric(numeric_data, errors='coerce').sum())
        except:
            return 0
    ...
    if queries.empty or 'Counts' not in queries.columns or queries['Counts'].isna().all():
        ...
        try:
            ...
            # ✅ FIX 2: KEEP NUMERIC COPY (NECESSARY - NOT A DUPLICATE!)
            ...
            if topN.empty:
                ...
                topN_numeric = topN_numeric[existing_columns]  # ✅ Keep numeric copy in sync
                ...
                if ('styled_top50_health' not in st.session_state or 
                    ...
                    # ✅ FIX 3: CORRECTED highlighting logic - compare with ORIGINAL numeric values from topN_numeric
                    def highlight_health_performance_with_comparison(styled_df):
                        ...
                        for i in range(1, len(sorted_months_list)):
                            ...
                            # ✅ FIXED: CTR comparison using ORIGINAL numeric values from topN_numeric
                            if current_ctr_col in topN_numeric.columns and prev_ctr_col in topN_numeric.columns:
                                for idx in topN_numeric.index:
                                    current_ctr = topN_numeric.loc[idx, current_ctr_col]  # ✅ Use topN_numeric
                                    prev_ctr = topN_numeric.loc[idx, prev_ctr_col]        # ✅ Use topN_numeric
                            ...
                            # ✅ FIXED: CR comparison using ORIGINAL numeric values from topN_numeric
                            if current_cr_col in topN_numeric.columns and prev_cr_col in topN_numeric.columns:
                                for idx in topN_numeric.index:
                                    current_cr = topN_numeric.loc[idx, current_cr_col]   # ✅ Use topN_numeric
                                    prev_cr = topN_numeric.loc[idx, prev_cr_col]         # ✅ Use topN_numeric
                ...
                # ✅ FIX 5: Fixed metrics calculation using safe_sum_column helper
                metrics = {
                    'total_queries': len(topN_numeric),
                    'total_search_volume': safe_sum_column(topN_numeric, 'Total Volume'),
                    'total_clicks': safe_sum_column(topN_numeric, 'Total Clicks'),
                    'total_conversions': safe_sum_column(topN_numeric, 'Total Conversions')
                }
                ...
                if unique_months:
                    ...
                    for month in unique_months:
                        ...
                        # ✅ FIX 3: Use topN_numeric for calculations
                ...
                csv = generate_csv_ultra(topN_numeric)  # ✅ Use numeric version for CSV
                ...
                with st.expander("📊 Monthly Performance Analysis", expanded=False):
                    if unique_months and len(unique_months) >= 2:
                        ...
                        with col1:
                            ...
                            ctr_improvements = []
                            # ✅ FIX 4: Use topN_numeric for calculations
                        ...
                        if len(unique_months) >= 2:
                            ...
                            # ✅ FIX 5: Use topN_numeric for all calculations
